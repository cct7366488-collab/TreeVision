...
| 狀態 | **accepted** |
| 草擬日期 | 2026-05-04 |
| 決議日期 | 2026-06-13（tree_id 命名、面積基準經試驗資料/主持人確認；#1/#3/#4 屬 TreeVision 內部可決或僅需知會主持人，不阻塞） |
...
| 實體 | 對應內容 |
|------|---------|
| `site` | 場域層級（事業區 + 林班 + 作業列區），如 `115-12-1`、`117-28-1`、`8-1-1`、`8-1-10`（短碼 `115121/117218/811/8110`） |
| `plot` | 試驗單元（在 site 內，依 treatment 切分），如 `115-12-1.P1F150`（修剪+施肥150g 組） |

`plot_id` 命名規則：`<site_id>.<treatment_id>`，例 `115-12-1.P1F150`
...
| 2 | ✅ **已解（2026-06-13）**：`樣區大小_m2 = 400` 係 `生長調查記錄表.xlsx` 空模板 P001 的**測試值**，非實際。**site 面積＝作業列區總面積**（115-12-1＝0.5 ha、117-28-1＝0.355 ha、8-1-1＝0.2372 ha、8-1-10＝0.4482 ha）；**監測 plot 若另設子樣區再補面積欄**，現階段不設。 | plot 粒度設計（已定案） |
| 3 | 計畫主持人是否接受「TreeVision 將樣區拆為 site + plot 兩層」？ | schema 接受度 |
| 4 | XLSX 中沒有「採集場次（campaign）」概念，TreeVision 引入是否會造成混淆？ | 概念對齊 |
| 5 | ✅ **已解（2026-06-13）**：真實資料樣木代碼＝`<樣區>-<處理>-<序號>`（例 `115121-C0-001`），**全域唯一**；多幹個體加莖序 `-s2/-s3`。`生長調查記錄表.xlsx` 的 T001 僅空模板測試值。tree_id 直接沿用此既有碼。 | tree_id 命名規則（已定案） |
...
| 2026-06-13 | 比對真實資料：未解 #5（tree_id 命名）已解＝`<樣區>-<處理>-<序號>` 全域唯一；#2（面積）已解＝site 面積為列區總面積、400 m² 為空模板測試值、plot 子樣區後補；site 範例更正 `1-2-3`→`8-1-1`；schema 對齊基準改對齊 `土肉桂試驗_長格式.xlsx`。**狀態 proposed → accepted**（#1 XLSX 升版／#3 site-plot 二層／#4 campaign 屬 TreeVision 內部可決或僅知會主持人，不阻塞）。 |
