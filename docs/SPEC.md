...
| 樣區 | 4 個官方林班：**115-12-1（成熟林）+ 117-28-1 / 8-1-1 / 8-1-10（幼齡林）** |
| 樹種 | 土肉桂（*Cinnamomum osmophloeum* Kanehira） |
| 林齡 | 1 個 15 年生 + 3 個 1~2 年生（八仙山兩區 2024/03 栽植） |
| 樣樹規模 | 約 480 株（4 樣區 × 4 處理 × ~30；死亡 21）〔2026-06-13 由試驗資料更正，原估 300〕 |
| 應用主軸 | 林下經濟（NTFP）試驗對照監測 |
| 試驗設計 | CRBD 雙因子析因 **全場 2×2**（修剪± × 施肥 150g±）〔2026-06-13 主持人確認，不含 200/250g〕 |
...
其中 `station_id = {tree_id}.{station_label}`（tree_id 已含樣區與處理），例：

```
115121-C0-001.north_side_20260601_1430_001.jpg
```
...
| `tree_id` | string | ✓ | `115121-C0-001` | 命名規範改（既有試驗碼） |
| `site_id` | string | ✓ | `115-12-1` | 對齊官方林班編號 |
| `plot_id` | string | ○ | `115-12-1.P1F150` | **NEW**（選填）|
| `treatment_id` | string | ○ | `P1F150` | **NEW**（選填）|
| `campaign_id` | string | ○ | `115-12-1_115Q1` | **NEW**（選填，site_season）|
| `station_id` | string | ○ | `115121-C0-001.north_side` | **NEW**（選填）|
...
| Tree | `tree_id`（既有碼，如 `115121-C0-001`；多幹莖序另欄） | 單棵樹個體 |
| TreeMeasurement | `tree_id` + `season` + `stem_seq` | 人工 / 影像基準量測（季配；多幹含莖序） |
...
| 實體 | 主鍵 | 說明 |
|------|------|------|
| Treatment | `treatment_id` | 處理組 `C0/P1/F150/P1F150`（全場 2×2，無 200/250g） |
| Plot | `plot_id` | 試驗單元（site × treatment 交集） |
| Campaign | `campaign_id` | 採集場次 |
| FixedCameraStation | `station_id` | 固定攝點（如 `115121-C0-001.north_side`） |
...
| 實體 | 主鍵 | 說明 |
|------|------|------|
| TreatmentResponseSummary | `plot_id` + `season` | 每處理組 × 場次（14 指標 + health_score 內含） |
...
Plot + Season 1 ── 1 TreatmentResponseSummary
Experiment + Metric 1 ── 1 ExperimentAnovaResult
```

詳細欄位定義見 [data-schema.md（v0.2）](data-schema.md) 與 [ADR-0003](decisions/0003-schema-alignment-with-xlsx.md)；已落地為 18 個 JSON schema（[`schemas/`](../schemas/)）+ 13 個 CSV 範本（[`metadata/`](../metadata/)），均經 jsonschema Draft 2020-12 與「土肉桂試驗_長格式」真實資料驗證。
...
- C-3：[**判釋鏈方法學與驗證框架——科學性佐證報告**（v0.2 NEW）](leaf-analysis-validation-report.md)
